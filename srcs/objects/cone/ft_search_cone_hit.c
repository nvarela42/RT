...
/*   Updated: 2018/05/08 15:22:08 by vmontagn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

void	ft_search_cone_hit(t_thread *th, t_objects *obj, t_ray *ray)
{
	float			dist_tmp;
	t_hit			obj_tmp;
	t_vector		vec_tmp;
	...
	if (obj->type == 4)
	{
		if (ft_calc_discri_cone(obj, ray, &obj_tmp))
		{
...
void	ft_lum_hit_cone(t_thread *th, t_objects *obj, t_ray *ray, int *block)
{
	float			dist_tmp;
	t_hit			obj_tmp;
	t_vector		vec_tmp;
